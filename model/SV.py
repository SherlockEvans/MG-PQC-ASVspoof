...
class Bottle2neck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.relu(out)
        out = self.bn1(out)
        # print("out.shape", out.shape)  torch.Size([2, 1024, 6435])
        spx = torch.split(out, self.width, 1)
        # print("spx[0].shape", spx[0].shape)  torch.Size([2, 128, 6435])
...
class RawNet3(nn.Module):
    def __init__(self, block, model_scale, context, summed, C=1024, **kwargs):
        ...
        self.summed = summed
        # print("kwargs[sinc_stride]",kwargs["sinc_stride"])
    ...
    def forward(self, x):
        ...
        if self.summed:
            x1 = self.layer1(x)
            x2 = self.layer2(x1)
            # print("x2.shape",x2.shape)   x2.shape torch.Size([2, 1024, 429])
            # print("self.mp3(x1) + x2",(self.mp3(x1) + x2).shape)   self.mp3(x1) + x2 torch.Size([2, 1024, 429])
            x3 = self.layer3(self.mp3(x1) + x2)
            # print("x3.shape", x3.shape)   x3.shape torch.Size([2, 1024, 429])
        else:
            x1 = self.layer1(x)
            x2 = self.layer2(x1)
            x3 = self.layer3(x2)
        ...
        x = self.relu(x)
        # print("x.shape", x.shape)  x.shape torch.Size([2, 1536, 429])
        t = x.size()[-1]
        ...
        # global_x.shape torch.Size([2, 4608, 429])
        w = self.attention(global_x)
        # print("w.shape",w.shape)  w.shape torch.Size([2, 1536, 429])
        ...
        x = torch.cat((mu, sg), 1)
        # print("x.shape", x.shape)   x.shape torch.Size([2, 3072])
        x = self.bn5(x)
...
if __name__ == "__main__":
    ...
    x = torch.randn(2, 64240).to(device)
    output = model(x)
